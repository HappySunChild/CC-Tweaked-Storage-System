local a={cache={}}do do local function b()local function c(d,e)local f=math.
floor(e/#d)local g=e%#d return function(h,i)local j=next(d,i)if j==nil then
return nil end local k=f+(g>0 and 1 or 0)g=g-1 if k==0 then return nil end
return j,h[j],k end,d,nil end return c end function a.a()local c=a.cache.a if
not c then c={c=b()}a.cache.a=c end return c.c end end do local function b()
local function c(d,e)for f,g in next,d do if g==e then return f end end return
nil end return c end function a.b()local c=a.cache.b if not c then c={c=b()}a.
cache.b=c end return c.c end end do local function b()local c=a.a()local d=a.b()
local e='Unable to find peripheral "%s"'local f=
...
rep(' ',f)if e<0 then return g..d end return d..g end return c end function a.f(
...
#d)end return c end function a.g()local c=a.cache.g if not c then c={c=b()}a.
cache.g=c end return c.c end end do local function b()local function c(d,e)if#d
<=e then return d end return(d:sub(1,math.max(e-3,1)):gsub('%s*$','')..'...'):
sub(1,e)end return c end function a.h()local c=a.cache.h if not c then c={c=b()}
a.cache.h=c end return c.c end end do local function b()local c=a.d()local d=a.
e()local e=a.f()local f=a.g()local g=a.h()local h='%d. 'local i={reconfigure=
...
return n end return k end function a.i()local c=a.cache.i if not c then c={c=b()
}a.cache.i=c end return c.c end end do local function b()local c=
...
format(f),2)end return g end return e end function a.j()local c=a.cache.j if not
c then c={c=b()}a.cache.j=c end return c.c end end do local function b()local c=
a.j()local d='Unable to find inventory "%s", is it being tracked?'local function
...
do j:track_inventory(l)end end return j end return h end function a.k()local c=a
.cache.k if not c then c={c=b()}a.cache.k=c end return c.c end end do
local function b()local c={AutoCrafter=a.c(),StorageDisplay=a.i(),ItemStorage=a.
k()}return c end function a.l()local c=a.cache.l if not c then c={c=b()}a.cache.
l=c end return c.c end end do local function b()local c='storage.io_inventory'
local d='storage.system_inventories'local e='storage.processors'local f=
...
.load()end,save=function()settings.save()end}end function a.m()local c=a.cache.m
if not c then c={c=b()}a.cache.m=c end return c.c end end do local function b()
...
end end table.sort(e)return e end return c end function a.o()local c=a.cache.o
if not c then c={c=b()}a.cache.o=c end return c.c end end do local function b()
...
register_pattern(i,j)end end return c end function a.p()local c=a.cache.p if not
c then c={c=b()}a.cache.p=c end return c.c end end do local function b()
...
==keys.enter then break end end return i end return c end function a.q()local c=
a.cache.q if not c then c={c=b()}a.cache.q=c end return c.c end end do
local function b()local c=a.q()local function d(e)local f={}for g,h in next,e do
...
end return n end return f end function a.r()local c=a.cache.r if not c then c={c
=b()}a.cache.r=c end return c.c end end do local function b()local c=a.q()
...
return d end function a.s()local c=a.cache.s if not c then c={c=b()}a.cache.s=c
end return c.c end end do local function b()local c=a.s()local function d(e,f,g)
table.insert(e,'Exit')while true do local h,i=c(e,f)if i==#e then break end g(i,
h)end end return d end function a.t()local c=a.cache.t if not c then c={c=b()}a.
cache.t=c end return c.c end end do local function b()local function c(d)return
function(e)local f=d.patterns[e]if f==nil then return'UNKNOWN PATTERN'end return
f.label end end return c end function a.u()local c=a.cache.u if not c then c={c=
b()}a.cache.u=c end return c.c end end do local function b()local c=a.t()local d
=a.r()local e=a.u()local f=a.p()local function g(h)return d(h:
...
elseif m==2 then i(j,k)elseif m==3 then f(k)end end)end end function a.v()local
c=a.cache.v if not c then c={c=b()}a.cache.v=c end return c.c end end do
local function b()local function c()os.pullEvent('key')os.pullEvent('key_up')end
return c end function a.w()local c=a.cache.w if not c then c={c=b()}a.cache.w=c
end return c.c end end do local function b()local c=a.t()local d=a.v()local e=a.
r()local f=a.m()local g=a.n()local h=a.e()local i=a.o()local j=a.w()
local function k(l)local m={}for n,o in next,l.processors do m[n]=o.patterns end
f.set(f.settings.PROCESSORS,m)f.save()end local function l(m)local n=m:
get_processors()if#n<=0 then term.clear()term.setCursorPos(1,1)term.write(
'No processors have been registered!')j()return end local o=e(n,
'Select processors to configure',h)if#o==0 then return end d(o,m)k(m)end
local function m(n,o,p,q)local function r()return g(i(n),function(s,t)return not
(t==o or p.inventories[t]~=nil or q.processors[t]~=nil)end)end local s=e(r(),
'Register Processors',h)for t,u in ipairs(s)do q:add_processor(u,{})end k(q)end
local function n(o)local p=e(o:get_processors(),'Deregister Processors',h)for q,
r in ipairs(p)do o:remove_processor(r)end k(o)end return function(o,p,q,r)c({
...
c end function a.y()local c=a.cache.y if not c then c={c=b()}a.cache.y=c end
return c.c end end do local function b()local c=a.t()local d=a.x()local e=a.r()
local f=a.s()local g=a.y()local h=a.e()local i=a.u()local j=a.w()local k=
...
clear()term.setCursorPos(1,1)local s=''local t=true for u,v in next,r do local w
=m:can_craft(u,v)if not w then t=false end s=s..string.format('%s %s -> %d\n',w
and'CRAFTABLE'or'MISSING',u,v)end if not t then printError(k:format(q,n(o),s))j(
)return end end local s=e(p,string.format(
'Select Processor Distribution | %dx %s',q,n(o)),h)if#s==0 then return end os.
queueEvent('start_crafting',s,o,q)end return function(m,n,o,p)c({'Craft',
'Processor inventories'},'Auto Processing Menu',function(q)if q==1 then l(p)
elseif q==2 then d(m,n,o,p)end end)end end function a.z()local c=a.cache.z if
not c then c={c=b()}a.cache.z=c end return c.c end end do local function b()
...
c end end do local function b()local c=a.t()local d=a.z()local e=a.s()local f=a.
y()local g=a.A()local h=a.e()local i=a.w()local j='Exporting %d items...'local k
='Importing items...'local l='Transferred %d items.'local function m(n,o,p)p.
setVisible(true)local function q(r)local s={}for t in next,o:get_system_items()
do local u,v=string.find(t,r)if v~=nil then table.insert(s,t:sub(v+1,-1))end end
return s end local function r()term.clear()p.redraw()local s=f(
'Storage System Output',"'exit' to return to menu",q)if s=='exit'then return
true end local t=o:get_system_items()local u={}for v in next,t do table.insert(u
,v)end local v=g(u,s)if v==nil then printError(string.format(
"Unable to find '%s'",s))i()return false end term.clear()p.redraw()local w=f(
string.format('Item Count - %s | %d in system',h(v),t[v]),"'cancel' to restart")
if w=='cancel'then return false end local x=tonumber(w)or 64 print(j:format(x))
local y=o:export_item(v,n,nil,x)print(l:format(y))i()end while true do if r()
then break end end p.setVisible(false)end local function n(o,p)local q,r=e({
'yesss!!!!!!','wait nvm'},'Storage System Input - Are you sure?')if r==2 then
return end term.clear()term.setCursorPos(1,1)print(k)local s=p:import_inventory(
o)print(l:format(s))i()end local function o(p)local q,r=term.getSize()term.
clear()term.setCursorPos(1,r)term.write('Press any key to return')p.setVisible(
true)i()p.setVisible(false)end return function(p,q,r,s,t,u)return function()c({
...
c end end end local b=a.l()local c=a.e()local d=a.m()local e=a.n()local f=a.o()
local g=a.p()local h=a.r()local i=a.s()local j=a.B()local k=peripheral.find(
'modem')d.load()local l=d.get(d.settings.IO_INVENTORY)local m=settings.get(d.
settings.SYSTEM_INVENTORIES)if l==nil or not k.isPresentRemote(l)then l=i(f(k),
'Choose IO Inventory',c)d.set(d.settings.IO_INVENTORY,l)end if m==nil then m=h(
e(f(k),function(n,o)return o~=l end),'Select System Inventories',c)d.set(d.
settings.SYSTEM_INVENTORIES,m)end d.save()local n=b.ItemStorage(m)local o=b.
AutoCrafter(n,d.get(d.settings.PROCESSORS))local p=peripheral.find('monitor')p.
setTextScale(0.5)local q,r=term.getSize()local s=window.create(term.current(),q*
2/3,2,math.ceil(q/3),r-2,false)local t=window.create(term.current(),1,1,q,r-1,
false)local u=b.StorageDisplay(p,{column_count=d.get(d.settings.MONITORS_COLUMNS
),index_justification=4})local v=b.StorageDisplay(s,{column_count=1,
index_justification=3})local w=b.StorageDisplay(t,{column_count=2,
index_justification=3})local function x()local y,z,A,B,C=os.pullEvent(
'start_crafting')local function D()o:start_batch_process_async(z,A,B)os.
queueEvent('crafting_finished',C)end parallel.waitForAll(D,x)end local function
y()while true do n:update_inventories()local z,A=term.getCursorPos()local B=n:
get_system_items_sorted()v:draw_item_cells(B)w:draw_item_cells(B)u:
draw_item_cells(B)term.setCursorPos(z,A)sleep(5)end end g(o)parallel.waitForAny(
j(k,l,n,o,s,t),x,y)